...
async function getRequest(requestNumber) {
  const records = await listJson(`crm-requests/${requestNumber}.json`, 1);
  return records[0] || null;
}

async function updateRequestStatus(requestNumber, patch) {
  const existing = await getRequest(requestNumber);
  if (!existing) throw new Error(`Request ${requestNumber} not found`);
  const record = {
    ...existing,
    ...patch,
    updatedAt: nowIso(),
  };
  return writeJson(`crm-requests/${requestNumber}.json`, record);
}
